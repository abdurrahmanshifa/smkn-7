...
class ArtikelController extends Controller
{
        ...
        if ($validator->passes()) {
            try{
                ...
                if($request->hasFile('cover'))
                {
                    ...
                    $cover = time()."_artikel_".Auth::user()->id."_".strtolower($file_name);
                    $file->storeAs('artikel', $cover);
                ...
                DB::commit();
                return redirect()->route('artikel.index')->with('success','Data Baru Artikel Berhasil Di Simpan');
            }
            catch(Exception $ex)
            {
                DB::rollback();
                return redirect()->route('artikel.create')->with('failed','Data Baru Artikel Gagal Di Simpan');
            }
        }
        else{
            return redirect()->route('artikel.create')->with('failed','Data Baru Artikel Gagal Di Simpan');
    ...
    public function update(Request $request , $id)
    {
        ...
        if ($validator->passes()) {
            try{
                ...
                DB::commit();
                return redirect()->route('artikel.index')->with('success','Data Baru Artikel Berhasil Di Edit');
            }
            catch(Exception $ex)
            {
                DB::rollback();
                return redirect()->route('artikel.update',$id)->with('failed','Data Baru Artikel Gagal Di Edit');
            }
        }
        else{
            return redirect()->route('artikel.update',$id)->with('failed','Data Baru Artikel Gagal Di Edit');
    ...
    {
        $data = Artikel::orderBy('tanggal','desc')->get();
        return Datatables::of($data)
            ->addIndexColumn()
            ->editColumn('aksi', function($data) {
                $hasil = '
                    <a href="'.route('artikel.edit',$data->id).'" data-toggle="tooltip" title="edit artikel" class="btn btn-warning btn-xs">
    ...
    public function edit_status(Request $request)
    {
        ...
        return redirect()->route('artikel.index')->with('success','Status Artikel Berhasil Di Edit');
    }
